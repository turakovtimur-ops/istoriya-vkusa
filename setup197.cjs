...
// ================= 2) RestaurantPage =================
let s = fs.readFileSync(P('src/components/RestaurantPage.tsx'), 'utf-8');
const s0 = s;
...
// иконки в подвале (колонка Контакты)
...
// кнопка Яндекс Еды рядом с «Забронировать стол» (только Кинза)
...
if (s !== s0) { fs.writeFileSync(P('src/components/RestaurantPage.tsx'), s, 'utf-8'); console.log('✓ RestaurantPage: VK+MAX в подвале, кнопка Еды у Кинзы'); }
else console.log('⚠ ничего не изменилось — пришли файл');
