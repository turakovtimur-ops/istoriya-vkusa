...
export default function SuppliersOrbit({ onPartner }: Props) {
  ...
  const tooltip = (s: Supplier) => (
    <div
      className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-60 glass-bar rounded-2xl p-4 text-left z-50"
    ...
    return (
      <div
        ...
        style={{
          left: p.x,
          top: p.y,
          transform: 'translate(-50%,-50%) scale(' + (0.8 + p.depth * 0.3) + ')',
          zIndex: p.depth > 0.5 ? 30 : 10,
      ...
      >
        <div className="relative flex flex-col items-center gap-1.5 w-32">
          ...
          <div
            className="relative rounded-full overflow-hidden border border-cream/20"
            style={{ width: size, height: size, boxShadow: '0 0 40px ' + s.accent + '66, 0 14px 34px rgba(0,0,0,0.5)' }}
          >
            {s.logo ? (
              <img src={s.logo} alt={s.name} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full bg-coal flex items-center justify-center">
                <span className="px-2 text-center leading-tight font-semibold text-cream/85 break-words" style={{ fontSize: size > 96 ? 11 : 12 }}>
                  {s.name}
                </span>
              </div>
            )}
    ...
    <div className="relative mx-auto" style={{ width: W, height: H }}>
      ...
      {(() => {
        ...
        return (
          <div
            className="absolute"
            style={{ left: p.x, top: p.y, transform: 'translate(-50%,-50%) scale(' + (0.8 + p.depth * 0.3) + ')', zIndex: p.depth > 0.5 ? 30 : 10, opacity: 0.75 + p.depth * 0.25 }}
          ...
          >
            <button onClick={onPartner} className="flex flex-col items-center gap-1.5 group">
              <div
                ...
                style={{
                  background: 'radial-gradient(circle at 32% 28%, #EBD3A8, #C2A076 55%, #8A6F4D)',
                  boxShadow: '0 0 44px rgba(194,160,118,0.55), 0 10px 26px rgba(0,0,0,0.5)',
                }}
              >
