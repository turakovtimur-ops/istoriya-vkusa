    ...
    <header className={'fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[min(1400px,calc(100vw-32px))] rounded-full transition-all duration-700 glass-bar ' + (scrolled ? 'scrolled py-3' : 'py-4')}>
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-6">
          <a href="#history" className="text-sm text-cream/70 hover:text-cream transition-colors">История</a>
          <a href="#restaurants" className="text-sm text-cream/70 hover:text-cream transition-colors">Рестораны</a>
          <a href="#promos" className="text-sm text-cream/70 hover:text-cream transition-colors">Акции</a>
          <a href="#events" className="text-sm text-cream/70 hover:text-cream transition-colors">Мероприятия</a>
          <a href="#suppliers" className="text-sm text-cream/70 hover:text-cream transition-colors">Партнёры</a>
          <a href="#vacancies" className="text-sm text-cream/70 hover:text-cream transition-colors">Вакансии</a>
          <a href="#contacts" className="text-sm text-cream/70 hover:text-cream transition-colors">Контакты</a>
        </nav>
      </div>
    </header>
  );
...
function PartnerPhoto({ p }: { p: Partner }) {
  ...
  return <img src={p.image} alt={p.name} onError={() => setFailed(true)} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />;
}
          ...
          <div className="lg:col-span-7 reveal reveal-delay-1">
            ...
            <div className="grid grid-cols-3 gap-6 mt-10 pt-8 border-t border-cream/10">
              {history.stats.map((s, i) => (
                <div key={i}>
                  <p className="text-5xl md:text-6xl font-semibold tracking-tighter text-amber">{s.value}</p>
                  <p className="text-xs uppercase tracking-[0.2em] text-cream/50 mt-2">{s.label}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            {restaurants.map((r, i) => (
              <a key={r.id} href={'#' + r.path} className="group reveal flex flex-col" style={{ transitionDelay: (i * 0.1) + 's' }}>
                ...
                <div className="mt-auto pt-4 border-t border-cream/10 space-y-1.5">
                  ...
                  <p className="text-xs text-cream/70">{r.phone}</p>
                </div>
