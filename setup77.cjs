...
// ---------- ВАКАНСИИ: целиком ----------
const vac = `import { useEffect, useState } from 'react';
...
export default function VacanciesOrbit({ onApply }: { onApply: (v: string) => void }) {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setMobileR(Math.min(window.innerWidth * 0.42, 190));
    };
    update();
...
`;
fs.writeFileSync(path.join(__dirname, 'src', 'components', 'VacanciesOrbit.tsx'), vac, 'utf-8');
console.log('✓ вакансии: мобильная орбита как на главной');

// ---------- ПОСТАВЩИКИ: целиком ----------
const sup = `import { useEffect, useRef, useState } from 'react';
...
`;
fs.writeFileSync(path.join(__dirname, 'src', 'components', 'SuppliersOrbit.tsx'), sup, 'utf-8');
console.log('✓ поставщики: мобильная орбита как на главной');

// ---------- CSS: карусели акций и партнёров, отступы ----------
let css = fs.readFileSync(path.join(__dirname, 'src', 'index.css'), 'utf-8');
if (css.includes('setup77')) {
  console.log('✓ CSS setup77 уже есть');
} else {
  fs.appendFileSync(path.join(__dirname, 'src', 'index.css'), `
/* ===== setup77: мобильные карусели + орбиты ===== */
@media (max-width: 767px) {
  #promos .grid, #partners .grid {
  ...
  #promos .grid::-webkit-scrollbar, #partners .grid::-webkit-scrollbar { display: none; }
  #promos .reveal, #partners .reveal { opacity: 1 !important; transform: none !important; }
  .marquee { display: none; }
  #suppliers .grid { display: none !important; }
  section { scroll-margin-top: 96px; }
}
`);
  console.log('✓ CSS setup77: карусели акций и партнёров (краешек следующей виден)');
}

console.log('\n✅ Дальше:\n   npm run build\n   git add . && git commit -m "Порядок блоков + мобильные карусели и орбиты" && git push');