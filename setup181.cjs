...
const MAX_UPLOAD = `async function maxUpload(base64: string, name: string, log?: any): Promise<any | null> {
  try {
...
`, 'utf-8');
console.log('✓ api/apply.ts: токен из up2');
...
`, 'utf-8');
console.log('✓ api/max-test.ts: токен из up2');

console.log('\n✅ Ритуал:');
console.log('npm run build');
console.log('git add -A && git commit -m "MAX: токен вложения из ответа загрузки"');
console.log('git pull --rebase');
console.log('git push');
console.log('\n📋 1) /api/max-test?key=iv2026 → send 200 + фото в чате');
console.log('2) Тестовая вакансия с фото → карточка + фото в чате!');